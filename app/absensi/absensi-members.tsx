...
export default function AbsensiMembersScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error('absensi-members load error:', e);
      } finally {
  ...
  const onSave = useCallback(async () => {
    ...
    try {
      // Ambil data lama
      const { data: existing } = await supabase.from('absensi_members').select('user_id');
      const existingIds = new Set((existing ?? []).map((r: any) => r.user_id as string));

      // Tentukan yang perlu dihapus dan ditambah
      ...
      if (toDelete.length > 0) {
        const { error } = await supabase.from('absensi_members').delete().in('user_id', toDelete);
        if (error) throw new Error(error.message);
      }
      if (toInsert.length > 0) {
        const { error } = await supabase.from('absensi_members').insert(toInsert.map(user_id => ({ user_id })));
        if (error) throw new Error(error.message);
      }
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor }]} edges={['top']}>
      ...
      <View style={[styles.header, { borderBottomColor: borderColor }]}>
        ...
        <View style={{ flex: 1, alignItems: 'center' }}>
          ...
          <ThemedText type="muted" style={{ fontSize: 12 }}>Berlaku untuk semua kegiatan</ThemedText>
        </View>
        <Pressable
          onPress={onSave}
          disabled={saving}
