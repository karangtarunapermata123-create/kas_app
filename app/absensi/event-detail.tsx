...
export default function EventDetailScreen() {
  ...
  const singleSession = useMemo(
    () =>
      event?.tipe === "SEKALI" && eventSessions.length >= 1
        ? eventSessions[0]
        : null,
    [event, eventSessions],
  );
  ...
  const [dbSingleSessionId, setDbSingleSessionId] = useState<string | null>(
    null,
  );
  useFocusEffect(
    useCallback(() => {
      ...
      supabase
        ...
        .then(({ data }) => {
          if (data && data.length > 0)
            setDbSingleSessionId((data[0] as any).id);
        });
    }, [eventId, event?.tipe]),
  );
  ...
  const loadManualYears = useCallback(async () => {
    ...
    try {
      ...
      console.error("loadManualYears error:", e);
    }
  }, [eventId]);
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      router.replace({
        pathname: "/absensi/session-detail",
        params: { sessionId },
      });
    }
  ...
  const prepareNextSessionDefaults = useCallback(() => {
    ...
    if (event.periodType === "MONTHLY") {
      ...
      if (evSessions.length > 0) {
        const last = [...evSessions].sort((a, b) =>
      ...
      const weekNum = Math.ceil(
        ((nextDate.getTime() - startOfYear.getTime()) / 86400000 +
          startOfYear.getDay() +
          1) /
          7,
      );
  ...
  const onConfirmAddYear = useCallback(async () => {
    ...
    try {
      await AsyncStorage.setItem(
        `manual_years_${eventId}`,
        JSON.stringify(newManualYears),
      );
    } catch (e) {
      console.error("saveManualYears error:", e);
    }

    // Langsung navigasi ke detail tahun tersebut
    if (event.periodType === "MONTHLY") {
      router.push({
        pathname: "/absensi/year-detail",
        params: { eventId: eventId, year: String(pendingNextYear) },
      });
    } else {
      router.push({
        pathname: "/absensi/weekly-year-detail",
        params: { eventId: eventId, year: String(pendingNextYear) },
      });
    }
  ...
  const showSessionFab =
    isAdmin && event?.tipe === "RUTIN" && event?.periodType !== "MONTHLY";
    ...
    <SafeAreaView style={[styles.safe, { backgroundColor }]} edges={["top"]}>
      ...
      <View style={[styles.header, { borderBottomColor: borderColor }]}>
        ...
        <View style={{ flex: 1, alignItems: "center" }}>
          ...
          <ThemedText type="muted" style={{ fontSize: 12 }}>
            {event?.tipe === "RUTIN"
              ? `Rutin · ${event.periodType === "WEEKLY" ? "Mingguan" : "Bulanan"}`
              : "Sekali"}
          </ThemedText>
        </View>
      ...
      {isRutin ? (
        ...
        >
          {yearGroups.length === 0 ? (
            ...
            yearGroups.map((yr) => {
              const pathname = isMonthly
                ? "/absensi/year-detail"
                : "/absensi/weekly-year-detail";
              return (
                <ThemedView
                  key={yr}
                    ...
                    <Pressable
                      ...
                      onPress={() =>
                        router.push({
                          ...
                          params: { eventId: eventId!, year: String(yr) },
                        })
                      }
                    >
                      ...
                      <View style={styles.sessionActions}>
                        <Pressable
                          onPress={() => {
                            ...
                            setRenameSessionValue(ses.label);
                          }}
                          style={({ pressed }) => [
                            styles.iconBtn,
                            { backgroundColor: tintColor + "15" },
      ...
      <Modal visible={confirmAddYear} transparent animationType="fade">
        <View style={styles.qrOverlay}>
          ...
          <ThemedView
            type="card"
            style={[styles.qrCard, { alignItems: "flex-start" }]}
          >
            <View
              style={[
            ...
            <ThemedText type="muted" style={{ fontSize: 13, marginBottom: 20 }}>
              untuk kegiatan ini
            </ThemedText>
            ...
            <View style={{ flexDirection: "row", gap: 12, width: "100%" }}>
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.btn,
                  { flex: 1, backgroundColor: tintColor },
                  pressed && { opacity: 0.8 },
                ]}
              >
      ...
      <Modal visible={addSessionVisible} transparent animationType="slide">
        <View style={styles.modalWrap}>
          ...
          <ThemedView
            type="card"
            style={[styles.modalCard, { maxHeight: height * 0.6 }]}
          >
            ...
            <View style={{ flexDirection: "row", gap: 12, width: "100%" }}>
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.btn,
                  { flex: 1, backgroundColor: tintColor },
                  (pressed || renamingSession) && { opacity: 0.8 },
                ]}
              >
                <ThemedText style={styles.btnText}>
                  {renamingSession ? "Menyimpan..." : "Simpan"}
                </ThemedText>
              </Pressable>
            </View>
          </ThemedView>
      ...
      <Modal visible={!!qrSession} transparent animationType="fade">
        <View style={styles.qrOverlay}>
          ...
          <ThemedView type="card" style={styles.qrCard}>
            <ThemedText
              type="defaultSemiBold"
              style={{ fontSize: 18, marginBottom: 4, textAlign: "center" }}
            >
              {qrSession?.label}
            </ThemedText>
            <ThemedText
      ...
      <Modal visible={!!deleteSessionTarget} transparent animationType="fade">
        <View style={styles.qrOverlay}>
          ...
          >
            ...
            >
              {isRutin
                ? `Hapus Tahun ${deleteSessionTarget?.label}`
                : "Hapus Sesi"}
            </ThemedText>
            <ThemedText
              type="muted"
                ...
                onPress={async () => {
                  ...
                  try {
                    if (isRutin) {
                      ...
                      const toDelete = eventSessions.filter((s) =>
                        s.tanggal.startsWith(`${yr}-`),
                      );
