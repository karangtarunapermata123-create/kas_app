...
const styles = StyleSheet.create({
  ...
  kasSelect: { flex: 1, flexDirection: 'row', alignItems: 'center', padding: 14, gap: 12 },
  kasIcon: { width: 40, height: 40, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  ...
  editModal: { width: '100%', borderTopLeftRadius: 28, borderTopRightRadius: 28, paddingTop: 8 },
  ...
  modalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 24, marginBottom: 4 },
  modalTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  modalIconBox: { width: 34, height: 34, borderRadius: 10, justifyContent: 'center', alignItems: 'center' },
  modalSubtitle: { fontSize: 12, marginTop: 2, paddingHorizontal: 24, marginBottom: 16 },
  segment: { flexDirection: 'row', marginHorizontal: 24, marginBottom: 4, backgroundColor: 'rgba(127,127,127,0.08)', borderRadius: 14, padding: 4 },
  segmentBtn: { flex: 1, paddingVertical: 9, alignItems: 'center', borderRadius: 11, flexDirection: 'row', justifyContent: 'center', gap: 5 },
  segmentText: { fontSize: 12, fontWeight: '600' },
  modalScrollContent: { paddingHorizontal: 24, paddingTop: 16, paddingBottom: 20 },
  sectionLabel: { fontSize: 12, fontWeight: '700', letterSpacing: 0.5, marginBottom: 10, textTransform: 'uppercase' },
  input: {
    borderWidth: 1.5,
    borderRadius: 12,
    paddingHorizontal: 16,
  ...
  editNameInput: { flex: 1, borderWidth: 1.5, borderRadius: 14, paddingHorizontal: 16, paddingVertical: 13, fontSize: 15 },
  saveNameBtn: { width: 48, height: 48, borderRadius: 14, justifyContent: 'center', alignItems: 'center' },
  memberCard: { flexDirection: 'row', alignItems: 'center', borderRadius: 14, marginBottom: 8, borderWidth: 1, paddingLeft: 14, paddingRight: 6, paddingVertical: 6 },
  ...
  addActionBtn: { width: 48, height: 48, borderRadius: 14, justifyContent: 'center', alignItems: 'center' },
  categoryCard: { borderRadius: 14, marginBottom: 8, borderWidth: 1, overflow: 'hidden' },
  categoryCardInner: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 10, gap: 10 },
  categoryNameInput: { flex: 1, fontSize: 14, fontWeight: '600', paddingVertical: 4 },
  categoryNominalInput: { width: 110, fontSize: 14, textAlign: 'right' },
  categoryDeleteBtn: { width: 32, height: 32, borderRadius: 9, justifyContent: 'center', alignItems: 'center' },
  saveCategoryBtn: { marginTop: 8, paddingVertical: 14, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  btn: { paddingVertical: 14, borderRadius: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10 },
  btnText: { color: 'white', fontWeight: '700', fontSize: 16 },
  typeSelector: { flexDirection: 'row', gap: 8, marginBottom: 12 },
  typeBtn: { flex: 1, paddingVertical: 10, alignItems: 'center', borderRadius: 12, borderWidth: 1, borderColor: 'rgba(127,127,127,0.2)' },
  typeBtnText: { fontSize: 13, fontWeight: '600' },
  modalCenterWrap: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 24 },
  modalFooterFixed: { paddingHorizontal: 24, paddingTop: 16, paddingBottom: 32, borderTopWidth: 1, borderTopColor: 'rgba(127,127,127,0.1)' },
  modalFooterFixedLandscape: { paddingHorizontal: 24, paddingTop: 12, paddingBottom: 16, borderTopWidth: 1, borderTopColor: 'rgba(127,127,127,0.1)' },
});
...
export default function KelolaBukuKasScreen() {
  ...
  const [newKasPeriodType, setNewKasPeriodType] = useState<PeriodType>('MONTHLY');
  const [addKasTab, setAddKasTab] = useState<'name' | 'members' | 'rates'>('name');
  const [newKasCats, setNewKasCats] = useState<{ name: string; nominal: string }[]>([]);
  ...
  const openEdit = useCallback((bId: string) => {
    ...
    cats.forEach(c => { initialRates[c] = String(rates[c] ?? b.periodConfig?.nominal ?? 0); });
    setRatesInput(initialRates);
    const initialNameEdits: Record<string, string> = {};
    cats.forEach(c => { initialNameEdits[c] = c; });
  ...
  const onSaveRates = useCallback(async () => {
    ...
    nextCategories.forEach(c => { normalizedRates[c] = parsedRates[c] ?? editBook.periodRates?.[c] ?? editBook.periodConfig?.nominal ?? 0; });
    await updatePeriodRates(editBookId, normalizedRates);
    closeEdit();
  }, [editBookId, editBook, categoryNameEdits, ratesInput, updateCategories, updatePeriodRates, closeEdit]);
  ...
  const onAddCategory = useCallback(async () => {
    ...
    setRatesInput(prev => ({ ...prev, [name]: String(isNaN(nominal) ? 0 : nominal) }));
    setCategoryNameEdits(prev => ({ ...prev, [name]: name }));
    setNewCategoryName('');
    setNewCategoryNominal('');
    setTimeout(() => newCategoryNameRef.current?.focus(), 50);
  }, [editBookId, editBook, newCategoryName, newCategoryNominal, updateCategories]);

  const onRemoveCategory = useCallback(async (cat: string) => {
    if (!editBookId) return;
    const next = (editBook?.categories || []).filter(c => c !== cat);
    await updateCategories(editBookId, next);
    setRatesInput(prev => { const copy = { ...prev }; delete copy[cat]; return copy; });
    setCategoryNameEdits(prev => { const copy = { ...prev }; delete copy[cat]; return copy; });
  ...
  const onAddKas = useCallback(async () => {
    ...
    try {
      if (newKasType === 'PERIODIK') {
        ...
        if (catsEntries.length === 0) return Alert.alert('Validasi', 'Tambahkan minimal satu kategori.');
        const firstNominal = catsEntries.find(c => !isNaN(c.nominal) && c.nominal > 0)?.nominal ?? 0;
      ...
      setNewKasName(''); setNewKasType('STANDARD'); setNewKasCats([]);
      setNewKasMembers([]); setNewMemberInput(''); setAddKasTab('name');
      setAddVisible(false);
    } catch (e: any) { Alert.alert('Gagal', e?.message ?? String(e)); }
  }, [newKasName, newKasType, newKasPeriodType, newKasCats, newKasMembers, addKas]);
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor }]} edges={['top']}>
        <View style={[styles.header, { borderBottomColor: borderColor }]}>
          ...
          <View style={styles.headerCenter}>
            <ThemedText type="defaultSemiBold" style={styles.headerTitle}>Kelola Buku Kas</ThemedText>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor }]} edges={['top']}>
      ...
      <View style={[styles.header, { borderBottomColor: borderColor }]}>
        ...
        <View style={styles.headerCenter}>
          <ThemedText type="defaultSemiBold" style={styles.headerTitle}>Kelola Buku Kas</ThemedText>
          <ThemedText type="muted" style={styles.headerSub}>{books.length} buku kas</ThemedText>
        </View>
        <View style={styles.headerRight} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {books.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <ThemedText type="muted" style={{ textAlign: 'center' }}>Belum ada buku kas.</ThemedText>
            <ThemedText type="muted" style={{ fontSize: 13, textAlign: 'center' }}>Tekan tombol + untuk menambahkan.</ThemedText>
          </View>
        ) : (
          books.map(b => (
            <View key={b.id} style={[styles.kasRow, { borderColor }]}>
              <View style={styles.kasSelect}>
                <View style={[styles.kasIcon, { backgroundColor: borderColor + '30' }]}>
                  <Ionicons name="journal-outline" size={20} color={mutedColor} />
                </View>
                <View style={{ flex: 1 }}>
              ...
              <Pressable
                ...
                style={({ pressed }) => [styles.actionBtn, { backgroundColor: tintColor + '12' }, pressed && { opacity: 0.6 }]}>
                <Ionicons name="create-outline" size={18} color={tintColor} />
              </Pressable>
              <Pressable
                onPress={() => setDeleteTarget({ id: b.id, nama: b.nama })}
                style={({ pressed }) => [styles.actionBtn, { backgroundColor: dangerColor + '12' }, pressed && { opacity: 0.6 }]}>
      ...
      <Modal visible={!!editBookId} transparent animationType="slide">
        <View style={{ flex: 1, justifyContent: 'flex-end' }}>
          <Pressable style={styles.modalOverlay} onPress={() => { Keyboard.dismiss(); closeEdit(); }} />
          <ThemedView type="card" style={[
            isLandscape ? styles.editModalLandscape : styles.editModal,
            { width: '100%', minHeight: 420, maxHeight: isLandscape ? height * 0.92 : (kbHeight > 0 ? height - kbHeight - 40 : Math.min(800, Math.round(height * 0.93))) },
          ]}>
            {!isLandscape && <View style={styles.dragHandle} />}
            <View style={styles.modalHeader}>
              <View style={styles.modalTitleRow}>
                <View style={[styles.modalIconBox, { backgroundColor: tintColor + '18' }]}>
                  <Ionicons name="create" size={17} color={tintColor} />
                </View>
                <ThemedText type="defaultSemiBold" style={{ fontSize: 17 }}>Edit Buku Kas</ThemedText>
              </View>
              <Pressable onPress={closeEdit} style={({ pressed }) => [styles.actionBtn, { backgroundColor: 'rgba(127,127,127,0.1)' }, pressed && { opacity: 0.6 }]}>
              ...
              {editBook && (
                <View>
                    ...
                    <View>
                      ...
                      <View style={styles.editNameRow}>
                        <TextInput
                          ...
                          style={[styles.editNameInput, { borderColor, color: textColor, backgroundColor: 'rgba(127,127,127,0.04)' }]}
                        />
                        <Pressable onPress={onSaveName}
                          style={({ pressed }) => [styles.saveNameBtn, { backgroundColor: tintColor }, pressed && { opacity: 0.8 }]}>
                  ...
                  {editBook.tipe === 'PERIODIK' && editTab === 'rates' && (
                    <View>
                      <ThemedText type="muted" style={styles.sectionLabel}>Kategori & Nominal</ThemedText>
                      {(editBook.categories || []).map(cat => (
                        <View key={cat} style={[styles.categoryCard, { borderColor }]}>
                          <View style={[styles.categoryCardInner, { backgroundColor: 'rgba(127,127,127,0.03)' }]}>
                            <View style={[styles.memberAvatar, { backgroundColor: tintColor + '12', marginRight: 0 }]}>
                              <Ionicons name="pricetag" size={14} color={tintColor} />
                            </View>
                            <TextInput
                              value={categoryNameEdits[cat] ?? cat}
                              onChangeText={v => setCategoryNameEdits(prev => ({ ...prev, [cat]: v }))}
                              placeholder="Kategori"
                              placeholderTextColor={mutedColor}
                              style={[styles.categoryNameInput, { color: textColor }]}
                            />
                            <TextInput
                              value={ratesInput[cat] ?? ''}
                              onChangeText={v => setRatesInput(prev => ({ ...prev, [cat]: v }))}
                              placeholder="Nominal"
                              placeholderTextColor={mutedColor}
                              keyboardType="number-pad"
                              style={[styles.categoryNominalInput, { color: textColor }]}
                            />
                            <Pressable onPress={() => onRemoveCategory(cat)}
                      ...
                      <Pressable onPress={onSaveRates}
                        style={({ pressed }) => [styles.saveCategoryBtn, { backgroundColor: tintColor }, pressed && { opacity: 0.85 }]}>
                        <Ionicons name="checkmark-circle" size={18} color="white" />
                        <ThemedText type="defaultSemiBold" style={{ color: 'white' }}>Simpan Perubahan</ThemedText>
                      </Pressable>
                    </View>
                  )}
            ...
            <View style={isLandscape ? styles.modalFooterFixedLandscape : styles.modalFooterFixed} />
          </ThemedView>
        </View>
      </Modal>
      ...
      <Modal visible={addVisible} transparent animationType="slide">
        <View style={{ flex: 1, justifyContent: 'flex-end' }}>
          <Pressable style={styles.modalOverlay} onPress={() => { Keyboard.dismiss(); setAddVisible(false); }} />
          <ThemedView type="card" style={[
            isLandscape ? styles.editModalLandscape : styles.editModal,
            { width: '100%', minHeight: 420, maxHeight: isLandscape ? height * 0.92 : (kbHeight > 0 ? height - kbHeight - 40 : Math.min(800, Math.round(height * 0.93))) },
          ]}>
            ...
            <View style={styles.modalHeader}>
              <View style={styles.modalTitleRow}>
                ...
                <ThemedText type="defaultSemiBold" style={{ fontSize: 17 }}>Tambah Buku Kas</ThemedText>
              </View>
              <Pressable onPress={() => setAddVisible(false)}
                style={({ pressed }) => [styles.actionBtn, { backgroundColor: 'rgba(127,127,127,0.1)' }, pressed && { opacity: 0.6 }]}>
                <Ionicons name="close" size={18} color={textColor} />
              </Pressable>
            </View>
            ...
            <ScrollView
              ...
              <View>
                  ...
                  <View>
                    ...
                    {newKasType === 'PERIODIK' && (
                      <View style={{ marginTop: 12 }}>
                        <ThemedText type="muted" style={styles.sectionLabel}>Periode Iuran</ThemedText>
                        <View style={styles.typeSelector}>
                          {(['MONTHLY', 'WEEKLY'] as const).map(p => (
                            <Pressable key={p} onPress={() => setNewKasPeriodType(p)}
                              style={[styles.typeBtn, newKasPeriodType === p && { backgroundColor: tintColor, borderColor: tintColor }]}>
                              <ThemedText style={[styles.typeBtnText, newKasPeriodType === p && { color: 'white' }]}>
                    ...
                    {newKasCats.map((cat, i) => (
                      <View key={i} style={[styles.categoryCard, { borderColor }]}>
                        <View style={[styles.categoryCardInner, { backgroundColor: 'rgba(127,127,127,0.03)' }]}>
                          ...
                          <ThemedText style={[styles.categoryNominalInput, { color: mutedColor }]}>{cat.nominal}</ThemedText>
                ...
                {/* Tombol Simpan */}
                <Pressable onPress={onAddKas}
                  style={({ pressed }) => [styles.btn, { backgroundColor: tintColor, marginTop: 20 }, pressed && { opacity: 0.85 }]}>
                  <Ionicons name="checkmark-circle" size={18} color="white" />
                  <ThemedText type="defaultSemiBold" style={styles.btnText}>Simpan Buku Kas</ThemedText>
                </Pressable>
              </View>
            </ScrollView>
          </ThemedView>
      ...
      <Modal visible={!!deleteTarget} transparent animationType="fade">
        <View style={styles.modalCenterWrap}>
          <Pressable style={[styles.modalOverlay, { position: 'absolute' }]} onPress={() => setDeleteTarget(null)} />
          <ThemedView type="card" style={{ width: isLandscape ? '70%' : '92%', padding: 24, borderRadius: 28 }}>
            <View style={{ alignItems: 'center', marginBottom: 20 }}>
              ...
              <ThemedText type="defaultSemiBold" style={{ fontSize: 20, marginBottom: 8 }}>Hapus Buku Kas</ThemedText>
              <ThemedText type="muted" style={{ textAlign: 'center', fontSize: 15, lineHeight: 22 }}>
                Hapus "{deleteTarget?.nama}"? Semua transaksi di dalamnya akan ikut terhapus dan tidak dapat dibatalkan.
              </ThemedText>
            </View>
