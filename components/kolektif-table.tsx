      ...
      {items.map((item, i) => {
        ...
        return (
          <Pressable
            ...
            style={({ pressed }) => [{
              borderWidth: 1, borderColor, borderRadius: 16,
              padding: 14, gap: 10,
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              <ThemedText type="defaultSemiBold" style={{ fontSize: 15, flex: 1 }} numberOfLines={1}>
                {item.nama}
              </ThemedText>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                <ThemedText type="muted" style={{ fontSize: 12 }}>{formatRupiah(item.nominal)}/orang</ThemedText>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: successColor + '15', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 }}>
                <Ionicons name="checkmark-circle" size={12} color={successColor} />
                <ThemedText style={{ fontSize: 11, color: successColor, fontWeight: '700' }}>{stat.paidCount} lunas</ThemedText>
              </View>
              {unpaidCount > 0 && (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: tintColor + '12', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 }}>
                  <Ionicons name="time-outline" size={12} color={tintColor} />
                  <ThemedText style={{ fontSize: 11, color: tintColor, fontWeight: '700' }}>{unpaidCount} belum</ThemedText>
                </View>
              )}
              {stat.skipCount > 0 && (
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: dangerColor + '12', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8 }}>
                  <Ionicons name="close-circle" size={12} color={dangerColor} />
                  <ThemedText style={{ fontSize: 11, color: dangerColor, fontWeight: '700' }}>{stat.skipCount} tidak setor</ThemedText>
                </View>
              )}
              <View style={{ flex: 1 }} />
              <ThemedText type="muted" style={{ fontSize: 11 }}>{Math.round(stat.pct * 100)}%</ThemedText>
            </View>
          </Pressable>
        );
      })}
