...
export async function loadKasState(): Promise<KasState> {
  try {
    ...
    console.log("loadKasState success:", {
...
export async function saveKasState(state: KasState): Promise<void> {
  try {
    const [booksRes, metaRes] = await Promise.all([
      ...
      supabase
        .from("meta")
        .upsert({ key: "activeKasId", value: state.activeKasId }),
    ]);
    if (booksRes.error)
      console.error("books upsert error:", JSON.stringify(booksRes.error));
    if (metaRes.error)
      console.error("meta upsert error:", JSON.stringify(metaRes.error));
  } catch (e) {
...
export function bookToRow(b: KasBook) {
  return {
    ...
    period_config:
      b.tipe === "KOLEKTIF"
        ? b.kolektifMode === "BULANAN"
          ? { kolektifMode: "BULANAN", kolektifTahun: b.kolektifTahun }
          : null
        : (b.periodConfig ?? null),
...
export function rowToBook(row: any): KasBook {
  return {
    ...
    nama: row.nama,
    tipe: row.tipe,
    periodConfig: row.tipe === "KOLEKTIF" ? undefined : row.period_config,
    ...
    kolektifMode:
      row.tipe === "KOLEKTIF"
        ? (row.period_config?.kolektifMode ?? "STANDARD")
        : undefined,
    kolektifTahun:
      row.tipe === "KOLEKTIF"
        ? (row.period_config?.kolektifTahun ?? undefined)
        : undefined,
...
export function txToRow(t: KasTx) {
  const nominal = typeof t.nominal === 'number' && !isNaN(t.nominal) ? t.nominal : 0;
  return {
    id: t.id,
    kas_id: t.kasId,
